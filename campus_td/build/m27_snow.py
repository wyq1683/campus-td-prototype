# m27_snow.py — 里程碑 M27：雪景冬季校园（Snowy Winter Campus · OptiX）
# 非破坏：不删/不改任何既有物体。仅新建 M27_ 前缀临时世界/物体（结束恢复），仅落盘 PNG。
# 思路：在已收官校园上叠加"冬季雪天"——冷调明亮天光 + 屋顶/地面积雪 + 空中飘雪粒子，
#   与 day / dusk(M6) / night(M24) / golden(M26) 形成第 5 种时段/天气氛围。复用 M19C/M11 机位。
...
# ---- 4) 冬季冷调明亮天光 ----
def make_snow_world():
    w = bpy.data.worlds.new("M27_SnowWorld")
    w.use_nodes = True
    nt = w.node_tree
    nt.nodes.clear()
...
def make_snow_material():
    ...
    # 5.x 新版 Principled：基色/糙度/金属输入名未变
...
bfl.location = (0.0, 0.0, -500.0)   # 模板移出画面，避免渲染出多余球体；粒子实例仍在各自位置出现
...
# 发射体自身不可见：套一层纯透明材质（粒子实例用 flake 自己的雪材质照常渲染）
...
try:
    ...
    cfg_samples()
    shots["hero"] = (os.path.join(OUT_DIR, "m27_snow_hero.png"),
                     render_shot(hero_cam, os.path.join(OUT_DIR, "m27_snow_hero.png")))
    shots["aerial"] = (os.path.join(OUT_DIR, "m27_snow_aerial.png"),
                       render_shot(aerial_cam, os.path.join(OUT_DIR, "m27_snow_aerial.png")))
finally:
    show(hidden_stack)


# ---- 11) 恢复白昼基线（不残留 M27_ 物体 / 雪世界 / 雪材质）----
def restore():
    # 先清 M27 几何/材质/相机
    for o in list(bpy.data.objects):
    ...
    # 再用 build_lighting 重建白昼基线（world/sun/exposure），避免旧 world 引用失效
    try:
        m06.build_lighting("day")
    except Exception as _e:
        print("restore build_lighting warning:", _e)
    # 清理所有 M27 命名的 world（含上回残留变体）
...
result = {
    ...
    "hero_cam_reused": ("M19C_HeroDay" if not hero_built else "M27_M19C_HeroDay"),
    "aerial_cam_reused": ("M11_CamAerial" if not aerial_built else "M27_M11_CamAerial"),
    "n_obj_after_restore": len(bpy.data.objects),
    "restored_world": sc.world.name,
}
print("M27_RESULT=" + repr(result))
