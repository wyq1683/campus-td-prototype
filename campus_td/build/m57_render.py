...
"""
M57 · 渲染（仅出图，不建物体）
复用 M52_Cam 门卫室特写 + M19C_HeroDay 校园语境，白昼 Cycles GPU OptiX。
...
try:
    ...
    if hero is not None:
        sc.camera = hero
        sc.render.filepath = PREVIEW_DIR + "/m57_guardhouse_details_hero.png"
    ...
    if cam is not None:
        ...
        bpy.context.view_layer.update()
        sc.render.filepath = PREVIEW_DIR + "/m57_guardhouse_details_closeup.png"
        bpy.ops.render.render(write_still=True)
        rendered.append(sc.render.filepath)
...
print("M57_RENDER=" + repr({"rendered": rendered, "n_m57_objects": n_m57}))
