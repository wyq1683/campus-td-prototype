...
"""
M58 · 渲染（仅出图，不建物体）
复用 M58_Cam 车道环线特写 + M19C_HeroDay 校园语境，白昼 Cycles GPU OptiX。
...
try:
    ...
    if hero is not None:
        sc.camera = hero
        sc.render.filepath = PREVIEW_DIR + "/m58_driveway_bluelines_hero.png"
    ...
    if cam is not None:
        ...
        bpy.context.view_layer.update()
        sc.render.filepath = PREVIEW_DIR + "/m58_driveway_bluelines_closeup.png"
        bpy.ops.render.render(write_still=True)
        rendered.append(sc.render.filepath)
...
print("M58_RENDER=" + repr({"rendered": rendered, "n_m58_objects": n_m58}))
